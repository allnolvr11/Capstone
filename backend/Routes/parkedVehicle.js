...
module.exports = (pool, secretKey) => {
    router.post('/parkedvehicles', (req, res) => {
        try {
            const copyQuery = `
                ...
                pool.query(displayQuery, (err, results) => {
                    if (err) {
    ...
    router.delete('/parkedvehicle/:parkingNumber', async (req, res) => {
        ...
        try {
            ...
            await pool.promise().execute('DELETE FROM parked_vehicles WHERE parking_number = ?', [parkingNumber]);
            
            // Retrieve session ID from parking_sessions table
            const [sessionResult] = await pool.promise().execute('SELECT session_id FROM parking_sessions WHERE parking_number = ?', [parkingNumber]);
            const sessionId = sessionResult[0].session_id;
            
            // Delete data from receipt_retrieval table
            await pool.promise().execute('DELETE FROM receipt_retrieval WHERE session_id = ?', [sessionId]);
            
            // Delete data from receipts table
            await pool.promise().execute('DELETE FROM receipts WHERE parking_session_id = ?', [sessionId]);
            
            // Delete data from parking_sessions table
            await pool.promise().execute('DELETE FROM parking_sessions WHERE parking_number = ?', [parkingNumber]);
